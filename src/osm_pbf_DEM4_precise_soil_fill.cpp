...
public:
    explicit DemSampler(const std::string& path) {
        ...
        int has_nodata = 0;
        nodata_value_ = band_->GetNoDataValue(&has_nodata);
        has_nodata_ = has_nodata != 0;

        int has_scale = 0;
        const double scale = band_->GetScale(&has_scale);
        scale_ = has_scale ? scale : 1.0;

        int has_offset = 0;
        const double offset = band_->GetOffset(&has_offset);
        offset_ = has_offset ? offset : 0.0;
    }
    ...
    double sample(double lon, double lat, double default_value = 0.0) const {
        ...
        if (x0 < 0 || y0 < 0 || x0 >= width_ || y0 >= height_) {
            return default_value;
        }

        if (x0 + 1 >= width_ || y0 + 1 >= height_) {
            return read_pixel(x0, y0, default_value);
        }

        const double dx = px - x0;
        const double dy = py - y0;
        const double v00 = read_pixel(x0, y0, default_value);
        const double v10 = read_pixel(x0 + 1, y0, v00);
        const double v01 = read_pixel(x0, y0 + 1, v00);
        const double v11 = read_pixel(x0 + 1, y0 + 1, v00);
        const double top = v00 * (1.0 - dx) + v10 * dx;
        const double bottom = v01 * (1.0 - dx) + v11 * dx;
        return top * (1.0 - dy) + bottom * dy;
    }

private:
    double read_pixel(int x, int y, double default_value) const {
        double value = default_value;
        const CPLErr err = band_->RasterIO(GF_Read, x, y, 1, 1, &value, 1, 1, GDT_Float64, 0, 0);
        if (err != CE_None) {
            return default_value;
        }
        if (has_nodata_ && std::abs(value - nodata_value_) < 1e-9) {
            return default_value;
        }
